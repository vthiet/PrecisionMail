...
* **Thành phần triển khai:** `AddAccountDialogController` đóng vai trò ConfigController/ConfigView trong sequence; dữ liệu được lưu tại bảng SQLite `accounts`.
...
* **Post-Condition(s) (Trạng thái hệ thống sau khi thành công):**
    ...
    1.  Các thông số cấu hình kết nối SMTP và IMAP được xác thực thành công bởi Mail Server.
    2.  Bản ghi cấu hình được mã hóa an toàn thông qua thuật toán AES-256/GCM và lưu trữ thành công vào cơ sở dữ liệu cục bộ SQLite.
...
2.  **S1.2:** Hệ thống truy vấn DB cục bộ:
    * *Trường hợp đã có cấu hình cũ:* Hệ thống sử dụng Crypto Service giải mã thông tin cấu hình và hiển thị dữ liệu lên form (riêng Mật khẩu ứng dụng được đặt trong `PasswordField`, nên JavaFX hiển thị dưới dạng ký tự ẩn khi render).
...
10. **S1.10:** Hệ thống chuyển giao mật khẩu ứng dụng thô cho dịch vụ mã hóa Crypto Service để mã hóa bằng thuật toán đối xứng AES-256/GCM với khóa dẫn xuất động.
...
* **EF-01-04: Lỗi ghi cơ sở dữ liệu cục bộ (Xảy ra tại bước S1.11):**
    ...
    * Database SQLite cục bộ bị khóa quyền ghi (do tiến trình khác chiếm dụng) hoặc ổ đĩa máy trạm bị đầy.
...
* **BR-01-01 (Mã hóa lưu trữ an toàn):** Mật khẩu ứng dụng (App Password) tuyệt đối không được ghi nhận dưới dạng văn bản thường (Plaintext) tại Database cục bộ hay bất kỳ tệp tin cấu hình tạm thời nào. Hệ thống sử dụng **AES-256/GCM**; khóa mã hóa được dẫn xuất bằng SHA-256 từ `security.aes.key` trong cấu hình ứng dụng kết hợp fingerprint máy cục bộ khả dụng trong JVM (`user.name`, `user.home`, `os.name`, `os.arch`, `os.version`). Các bản mã legacy AES/ECB cũ chỉ được hỗ trợ để giải mã tương thích và sẽ được ghi lại theo định dạng AES-GCM khi lưu cấu hình.
* **BR-01-02 (Tự động gợi ý cổng Port thông minh):** Để giảm thiểu lỗi nhập liệu từ người dùng, khi người dùng lựa chọn Giao thức mã hóa bảo mật trên form, hệ thống tự động điền gợi ý các cổng Port tiêu chuẩn:
...
* **NFR-01-02 (Hiệu năng phản hồi nút bấm):** Thời gian phản hồi xử lý sự kiện đóng mở form và click nút phải nhỏ hơn 300 ms trên luồng UI. Thời gian tối đa cho một tiến trình kiểm tra mạng kết nối bị giới hạn bởi timeout 10 giây cho connect/read/write.
* **NFR-01-03 (Dọn dẹp bộ nhớ đệm):** Sau khi luồng kiểm tra kết nối hoàn tất hoặc bị hủy bỏ, hệ thống đóng `Transport`/`Store` Jakarta Mail bằng try-with-resources và hủy tham chiếu tác vụ nền để giải phóng tài nguyên mạng.
...
sequenceDiagram
    ...
    participant ConfigController as AddAccountDialogController
    ...
    participant LocalDB as Local Database (SQLite)
    ...
    alt Có tồn tại dữ liệu cũ trong DB
        ...
        Note over ConfigController: Đặt mật khẩu vào JavaFX PasswordField<br/>UI render dưới dạng ký tự ẩn
    end
    ...
    CryptoService-->>ConfigController: Trả về chuỗi mật khẩu đã mã hóa (AES-256/GCM)
