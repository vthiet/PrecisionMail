...
9.  **S2.9:** Hệ thống vô hiệu hóa tạm thời các nút bấm trên biểu mẫu soạn thảo (Gửi, Đính kèm, Hủy soạn, Lên lịch gửi) để tránh người dùng thao tác trùng lặp. Đồng thời, khởi tạo một tiến trình bất đồng bộ ngầm bằng `CompletableFuture` trên Virtual Thread executor của JDK 21.
10. **S2.10:** Hệ thống sử dụng tài khoản đang được chọn trên giao diện. Danh sách tài khoản đã được tải từ DB cục bộ và giải mã App Password trước đó thông qua `AccountService`/`CryptoUtil` khi màn hình soạn thư khởi tạo hoặc khi cấu hình tài khoản thay đổi.
...
* **NFR-02-01 (Xử lý bất đồng bộ ngầm - Multi-threading):** Tác vụ soạn thảo và bấm nút gửi hoàn toàn không được gây ảnh hưởng tới luồng xử lý đồ họa chính (JavaFX Application Thread). Việc gửi thư được thực hiện bằng `CompletableFuture` chạy trên Virtual Thread executor; kết quả gửi được marshal về JavaFX Application Thread bằng `Platform.runLater` để cập nhật giao diện. Giao diện người dùng phải giữ nguyên phản hồi 60 FPS, cho phép thu nhỏ cửa sổ hoặc chuyển sang xem trang lịch sử trong lúc tiến trình gửi ngầm đang chạy.
...
sequenceDiagram
    ...
    %% 0. TẢI CẤU HÌNH TÀI KHOẢN GỬI
    ComposeController->>AccountService: findAll()
    AccountService->>LocalDB: loadAccountConfiguration()
    LocalDB-->>AccountService: Trả về thông tin tài khoản đã mã hóa
    AccountService->>CryptoUtil: decrypt(encryptedPassword)
    CryptoUtil-->>AccountService: Trả về App Password thuần (Plaintext)
    AccountService-->>ComposeController: Trả về danh sách tài khoản đã giải mã
    ...
    Note over ComposeController, EmailService: Khởi tạo CompletableFuture trên Virtual Thread ngầm<br/>để kết nối Socket SMTP, giữ UI mượt mà
    
    ComposeController->>EmailService: sendAsync(selectedAccount, emailDetails)
