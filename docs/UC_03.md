...
* **Post-Condition(s) (Trạng thái hệ thống sau khi thành công):**
    ...
    1.  Bản ghi thông tin Email được ghi nhận thành công vào Database cục bộ với trạng thái ban đầu là "Chờ gửi" (`SCHEDULED`) cùng mốc thời gian gửi dự kiến ($T_{\text{target}}$).
...
2.  **S3.2:** Hệ thống hiển thị khu vực chọn thời gian trên màn hình soạn thư (DatePicker + Hour/Minute ComboBox).
...
5.  **S3.5:** Thời gian hợp lệ, hệ thống khóa tạm thời thao tác lập lịch đang xử lý trên form soạn thảo chính.
6.  **S3.6:** Hệ thống lưu trữ thông tin email đã soạn cùng thông tin lịch trình ($T_{\text{target}}$) vào DB cục bộ dưới trạng thái "Chờ gửi" (`SCHEDULED`).
7.  **S3.7:** Hệ thống đăng ký tác vụ định thời theo `scheduled_email.id` vào bộ nhớ đệm và hiển thị thông báo "Đã lên lịch gửi email thành công\!". Biểu mẫu soạn thảo được làm sạch để người dùng có thể soạn thư tiếp theo.
8.  **S3.8:** Bộ định thời ngầm (Scheduler Daemon) chạy trên luồng daemon của JVM; khi ứng dụng khởi động, bootstrap process quét DB và đăng ký lại toàn bộ bản ghi `SCHEDULED`/`RETRY_PENDING` còn hợp lệ.
...
12. **S3.12:** Hệ thống cập nhật trạng thái bản ghi trong DB cục bộ thành `SENT`, ghi log hệ thống dạng INFO và bản ghi xuất hiện trong danh sách lịch sử khi người dùng mở hoặc làm mới màn hình lịch sử gửi.
...
* **AF-03-01: Chỉnh sửa hoặc Hủy bỏ lịch gửi trước giờ G:**
    ...
    * Người dùng chọn một bản ghi email đang ở trạng thái "Chờ gửi".
      * Hệ thống cung cấp hai lựa chọn:
          ...
          2.  *Thay đổi thời gian:* Hệ thống cho phép chọn mốc thời gian mới ($T_{\text{new}}$) bằng DatePicker + Hour/Minute ComboBox. Hệ thống thực hiện cập nhật lại mốc thời gian trong DB và tái cấu hình tác vụ định thời ngầm.
...
$$
    * Hệ thống từ chối xác nhận, hiển thị cảnh báo: *"Scheduled time must be at least 60 seconds in the future."* và giữ nguyên màn hình để người dùng thiết lập lại.
...
$$
    * Hệ thống sẽ tự động chuyển trạng thái của các bản ghi này thành "Trễ hạn" (`MISSED`). Đồng thời ghi nhận log sự kiện ở cấp độ WARN để người dùng theo dõi.
...
* **EF-03-03: Mất mạng Internet tại thời điểm kích hoạt tự động gửi (Xảy ra tại bước S3.10):**
    ...
    * Thay vì đánh dấu thất bại ngay lập tức, hệ thống áp dụng chính sách thử lại tự động (Retry Policy): Thực hiện gửi lại tối đa 3 lần, mỗi lần cách nhau $5$ phút ($300 \text{ giây}$).
    * Nếu sau 3 lần thử lại vẫn thất bại, hệ thống dừng tiến trình gửi, cập nhật trạng thái bản ghi trong DB cục bộ thành `FAILED` với thông điệp "Network connection error after retry policy was exhausted" kèm chi tiết lỗi và ghi nhận log WARNING/ERROR.
...
sequenceDiagram
    ...
    Note over ScheduleController: Xác thực logic thời gian chuẩn IEEE:<br/>Kiểm tra nếu $T_{target} \ge T_{current} + 60 \text{ giây}$
    
    ScheduleController->>LocalDB: saveScheduledEmail(emailData, targetTime, STATUS_SCHEDULED)
    ...
    Note over SchedulerService: Cập nhật hàng đợi định thời ngầm<br/>theo taskId ánh xạ 1:1 với DB
    
    ScheduleController-->>ScheduleView: Phản hồi "Lên lịch thành công"
    ScheduleView-->>User: Hiển thị trạng thái Email: "Đang chờ gửi" (SCHEDULED)

    %% GIAI ĐOẠN 2: THỰC THI TỰ ĐỘNG THEO THỜI GIAN (BACKGROUND PROCESS)
    loop Tác vụ định thời daemon
        SchedulerService->>SchedulerService: waitUntilTargetTime(taskId)
    end
    ...
    %% Cập nhật giao diện khi người dùng mở hoặc làm mới lịch sử
    opt Người dùng mở/làm mới màn hình lịch sử
        ScheduleView->>LocalDB: loadSentHistory()
        LocalDB-->>ScheduleView: Trả về bản ghi đã gửi (STATUS_SENT)
        ScheduleView-->>User: Hiển thị Email trong mục "Đã gửi" (Sent)
    end
```
