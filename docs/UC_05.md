...
* **Description (Mô tả chức năng):** Cung cấp giao diện trực quan cho phép người dùng xem danh sách, lọc, tìm kiếm nâng cao và truy vấn chi tiết các email đã được xử lý từ cơ sở dữ liệu SQLite cục bộ. Đồng thời hỗ trợ tính năng xuất báo cáo danh sách đang hiển thị ra định dạng tệp tin di động.
...
* **Pre-Condition(s) (Điều kiện tiên quyết):**
    ...
    2.  Cơ sở dữ liệu SQLite cục bộ hoạt động bình thường và sẵn sàng tiếp nhận các truy vấn đọc (Read-only queries).
...
5.  **S5.5:** Người dùng nhấn nút **"Tìm kiếm"** hoặc nhấn Enter tại ô từ khóa.
...
* **AF-05-01: Xuất báo cáo lịch sử ra tệp tin CSV:**
    ...
    * Hệ thống tạo luồng ngầm thực hiện đọc tập dữ liệu đang hiển thị hiện tại trên trang hiện hành, chuyển đổi định dạng sang chuẩn CSV (mã hóa ký tự UTF-8 có BOM để tránh lỗi hiển thị tiếng Việt trên MS Excel) và ghi xuống ổ đĩa cục bộ.
    * Xuất tệp thành công, hệ thống hiển thị thông báo popup trực quan: *"Xuất dữ liệu lịch sử thành công\!"* kèm đường dẫn tệp đã lưu.
...
* **NFR-05-03 (Tối ưu hóa RAM và giải phóng WebView):**
    ...
    * Ngay khi người dùng đóng hộp thoại popup chi tiết thư, hệ thống bắt buộc phải gọi phương thức hủy tham chiếu, dọn dẹp bộ nhớ đệm WebView và gợi ý Garbage Collector (System.gc()) để tránh hiện tượng rò rỉ bộ nhớ (Memory Leak) làm treo ứng dụng sau nhiều lần sử dụng.
...
sequenceDiagram
    ...
    alt Trường hợp T_start > T_end (Sai logic thời gian)
        ...
        Note over LocalDB: Quét dữ liệu tối ưu qua các trường<br/>đã được đánh chỉ mục (Index):<br/>sent_at, status, recipient
        LocalDB-->>HistoryService: Trả về danh sách Bản ghi (Metadata gồm: ID, To, Subject, Time, Status)
