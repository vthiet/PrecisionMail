...
2.  **S4.2:** Hệ thống gửi câu lệnh truy vấn tới Database cục bộ để tìm kiếm tất cả các bản ghi email có trạng thái là "Chờ gửi" (Pending).
3.  **S4.3:** Hệ thống hiển thị danh sách hàng đợi lên bảng (TableView của JavaFX) được sắp xếp thứ tự ưu tiên theo thời gian dự kiến gửi () tăng dần, kèm theo các nút chức năng: "Xem chi tiết", "Chỉnh sửa", "Hủy gửi".
...
*   **AF-04-01: Chỉnh sửa nội dung và Thời gian gửi của Email trong hàng đợi:**
    ...
    *   Người dùng tiến hành thay đổi các thông tin (Người nhận, Tiêu đề, Nội dung rich-text, Tệp đính kèm hoặc Ngày/Giờ gửi mới $T_{\text{new}}$) và nhấn **"Xác nhận cập nhật"**.
...
sequenceDiagram
    ...
    LocalDB-->>QueueController: Trả về danh sách Email đang chờ (STATUS_PENDING)
