...
2.  **S6.2:** Hệ thống kích hoạt một tiến trình ngầm (Virtual Thread) thực hiện đọc bất đồng bộ $N_{\text{lines}} = 1000$ dòng nhật ký mới nhất từ tệp tin log hiện hành nằm trong thư mục cài đặt của ứng dụng.
...
10. **S6.10:** Hệ thống gọi thư viện Desktop API của Java để kích hoạt trình quản lý tệp tin mặc định của Hệ điều hành (Windows Explorer hoặc Linux Files/Nautilus) mở trực tiếp thư mục chứa file log gốc.
...
%% GIAI ĐOẠN 1: TẢI ĐUÔI LOG BAN ĐẦU (TAIL RECENT LOGS)
    ...
    LogController->>LogMonitoringService: fetchRecentLogs(maxLines=1000)

    LogMonitoringService->>LogbackEngine: getActiveLogFilePath()
    LogbackEngine-->>LogMonitoringService: Trả về đường dẫn (e.g., logs/sys_app.log)

    LogMonitoringService->>FileSystem: readLastNLines(filePath, 1000)
...
%% GIAI ĐOẠN 2: LỌC LOG THEO TIÊU CHÍ (DYNAMIC FILTERING)
    ...
    LogController->>LogMonitoringService: streamAndFilterLogs(level, keyword)
...
%% GIAI ĐOẠN 3: XUẤT FILE LOG (EXPORT LOG REPORT)
    ...
    LogMonitoringService->>FileSystem: copyOrCreateZip(sourceLogPath, destinationPath)
    ...
    LogView-->>User: Hiển thị Popup "Xuất file báo cáo Log thành công!"
```