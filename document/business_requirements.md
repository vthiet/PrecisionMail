...
- **FR1.2. Lưu trữ bảo mật:** Ứng dụng phải lưu thông tin đăng nhập vào một tệp cấu hình (file) duy nhất trên local, hỗ trợ thao tác thêm/xóa tài khoản. Dữ liệu mật khẩu trong tệp phải được mã hóa.
- **FR1.3. Chọn đường dẫn lưu file:** Người dùng có quyền chọn thư mục/vị trí trên máy tính để tạo/lưu tệp cấu hình chứa tất cả tài khoản này.
- **FR1.4. Đa tài khoản:** Ứng dụng cho phép lưu nhiều tài khoản chung trong tệp cấu hình và cung cấp giao diện để người dùng dễ dàng chuyển đổi qua lại (Switch accounts) giữa các tài khoản người gửi.
...
- **FR2.3. Tệp đính kèm:**  Cho phép đính kèm tệp tin.
    ...
    - Quy tắc ràng buộc: Hệ thống chặn tự động các tệp tin thực thi có nguy cơ bảo mật như `.exe`, `.bat`, `.cmd`, `.vbs` để tránh bị SMTP server từ chối.

### FR3. Bulk Sending

- **FR3.1. Nhập danh sách:** Hỗ trợ tính năng Import danh sách email người nhận từ tệp văn bản (.txt) hoặc tệp bảng tính (.csv).
- **FR3.2. Phân tích tệp:** Ứng dụng cần đọc và trích xuất hợp lệ các địa chỉ email từ tệp danh sách. Hỗ trợ cả hai phương thức: (1) Giả định mỗi dòng là 1 địa chỉ email duy nhất, và (2) Sử dụng Regex để tự động tìm và trích xuất email từ dòng có nhiều cột dữ liệu.

### FR4. Scheduling & Execution

- **FR4.1. Lên lịch:** Cho phép người dùng thiết lập ngày, giờ, phút, giây chính xác để gửi email. Thời gian lên lịch tối đa là trong vòng 2 giờ (yêu cầu người dùng phải giữ ứng dụng mở).
- **FR4.2. Cơ chế Sleep & Wake:** Khi người dùng bấm "Lên lịch/Chuẩn bị", hệ thống sẽ chuyển tác vụ vào trạng thái "ngủ" (Sleep). Khi gần đến sát giờ gửi đã định, hệ thống mới "thức dậy" để tạo sẵn và duy trì một kết nối (Session/Transport) mở tới máy chủ Google SMTP nhằm tránh việc bị ngắt kết nối do Timeout.
- **FR4.3. Kích hoạt trễ thấp:** Đúng thời điểm đã lên lịch, hệ thống đẩy gói dữ liệu đi ngay lập tức trên kết nối vừa mở để tối thiểu hóa độ trễ.
...
- **NFR1. Security:** Tệp lưu trữ tài khoản local phải được mã hóa bằng thuật toán chuẩn (ví dụ: AES) để người dùng khác mở file không đọc được App Password. Khóa bí mật (Secret Key) để mã hóa/giải mã được lưu cố định trong tệp `application.properties`.
